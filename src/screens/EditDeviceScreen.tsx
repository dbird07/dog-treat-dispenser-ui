...
export const EditDeviceScreen: React.FC<EditDeviceScreenProps> = ({ navigation, route }) => {
  ...
  useEffect(() => {
    if (device) {
      setFormData({
        name: { value: device.name, error: '', touched: false },
        capacity: { value: device.capacity, error: '', touched: false },
        treatsRemaining: { value: device.treatsRemaining, error: '', touched: false },
        frequencyMode: device.schedule.mode,
        everyNth: { value: device.schedule.everyNth || 2, error: '', touched: false },
        probability: { value: device.schedule.probability || 0.7, error: '', touched: false },
        fadeEnabled: device.schedule.fadeEnabled,
        tonePreset: device.tonePreset || 'whistle',
        isOn: device.isOn,
      });
    }
  }, [device]);
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.header, { backgroundColor: theme.colors.surface, borderBottomColor: theme.colors.border }]}>
        ...
        <Text style={[styles.title, { color: theme.colors.text }]}>Edit Device</Text>
